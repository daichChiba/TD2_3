...
void PlayerAccelerationBullet::Update() {
	if (!start) {
	...
	if (currentTime < duration) {
		// イージングを適用して弾の位置を更新
		Vector3 newPosition = Easings::EaseInAcceleration(startPos, tagetPos, currentTime, duration);
		newPosition.z = 0.0f;
...
void PlayerAccelerationBullet::DrowImgui() {
#ifdef _DEBUG
